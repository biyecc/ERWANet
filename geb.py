...
class EGBBlock(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mlp = nn.Sequential(Linear(4*in_channels, 2*out_channels),
                                 nn.ReLU,
                                 Linear(2*out_channels, 2*out_channels))
    ...
    def message(self, x_j: Tensor, x_i:Tensor, alpha: Tensor) -> Tensor:
        edge_fea = torch.cat([x_i, x_j, x_j-x_i], dim=1)
        x = torch.cat((x_j, edge_fea),-1)
        x = self.mlp(x)
